...
class Example(object):
    ...
    def __init__(self, title, summarization, vocab, hps):
        ...
        # title_words = split_text_with_whitespace(title) + [MARK_EOS]
        title_words = ' '.join(title)
        title_words = title_words.split(' ') + [MARK_EOS]
        # if len(title_words) > hps.max_enc_steps:
        #     title_words = title_words[:hps.max_enc_steps]
        self.enc_len = len(title_words)
        self.enc_input = [vocab.word2id(w) for w in title_words]

        # Process the summarization
        #summarization_words = split_text_with_whitespace(summarization)
        summarization_words = ' '.join(summarization)
        summarization_words = summarization_words.split(' ')
    ...
    def init_encoder_seq(self, example_list, hps):
        ...
        if hps.pointer_gen:
            ...
            self.max_art_oovs = max(
                [len(ex.title_oovs) for ex in example_list])
            # Store the in-article OOVs themselves
            self.art_oovs = [ex.title_oovs for ex in example_list]
...
class Batcher(object):
    ...
    def fill_example_queue(self):
        ...
        while True:
            try:
                (title, summarization) = input_gen.next()
            ...
            example = Example(title, summarization, self._vocab,
