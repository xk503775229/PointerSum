...
tf.app.flags.DEFINE_string(
    "data_path", "/home/kxiao/kangjia/works_ex3/pointer_train_data.txt", "Path expression to tf.Example datafiles. Can include wildcards to access multiple datafiles."
)
tf.app.flags.DEFINE_string("vocab_path", "/home/kxiao/Pointer4Summ/pointerVocab.txt", "Path expression to text vocabulary file.")
...
tf.app.flags.DEFINE_boolean(
    ...
    "If True, run eval on the full dataset using a fixed checkpoint, i.e. take the current checkpoint, "
    "and use it to produce one summary for each example in the dataset, "
...
tf.app.flags.DEFINE_string(
    "exp_name", "myexperiment2", "Name for experiment. Logs will be saved in a directory with this name, under log_root."
)
...
tf.app.flags.DEFINE_integer(
    "min_dec_steps", 1, "Minimum sequence length of generated summary. Applies only for beam search decoding mode"
)
tf.app.flags.DEFINE_integer(
    "vocab_size",
    30000,
...
tf.app.flags.DEFINE_boolean(
    "restore_best_model",
    False,
    "Restore the best model in the eval/ dir and save it in the train/ dir, ready to be used for further training."
    "Useful for early stopping, or if your training checkpoint has become corrupted with e.g. NaN values.",
)
...
def setup_training(model, batcher):
    ...
    if FLAGS.convert_to_coverage_model:
        assert (
            FLAGS.coverage
        ), "To convert your non-coverage model to a coverage model, run with convert_to_coverage_model=True and coverage=True"
        convert_to_coverage_model()
...
def run_eval(model, batcher, vocab):
    ...
    while True:
        _ = util.load_ckpt(saver, sess, "train")  # load a new checkpoint
        ...
        if best_loss is None or running_avg_loss < best_loss:
            tf.logging.info(
                "Found new best model with %.3f running_avg_loss. Saving to %s", running_avg_loss, bestmodel_save_path
            )
...
def main(unused_argv):
    ...
    # todo: update FLAGS.log_root may cause bug, need to fix
